...
assert.match(playerPage, /const title = 'Fight History'/, 'player profiles expose the hockey-native fan-voted Fight History');
assert.match(playerPage, /data-fight-scope=/, 'Fight History has explicit season tabs (each ledger season + Career)');
assert.match(playerPage, /intel\(`\/fights\/player\/\$\{id\}`/, 'fight history, stat strip and card read the one nhl-metrics fight ledger');
