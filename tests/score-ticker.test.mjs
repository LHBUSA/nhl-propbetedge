...
// --- mounted through the shell lifecycle, not a page hack --------------
assert.match(shell, /<div id="score-ticker-slot"><\/div>/, 'the rail has one shell mount point');
assert.match(shell, /<\/header>\s*\n\s*<div id="score-ticker-slot">/, 'the rail sits directly below the top navigation');
assert.match(shell, /score-ticker-slot"><\/div>\s*\n\s*<div class="backdrop-wrap"/, 'the rail sits above the backdrop, the mode ribbon and page content');
assert.match(main, /mountScoreTicker\(document\.querySelector\('#score-ticker-slot'\), ctx\)/, 'the rail is booted once from the app lifecycle');
assert.match(main, /styles\/score-ticker\.css/, 'rail styles ship with the shell');

console.log('score ticker: PASS — one NHL data path, normalized semantics, stale honesty, reduced motion, shell-mounted');
