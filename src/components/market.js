...
export function marketPanel(ev, meta) {
  ...
  return `<div class="market-panel">
    ...
    <p class="micro market-note">Market snapshot (${esc(meta?.source || 'The Odds API')}), scheduled 08:00 / 13:00 / 18:00 ET — not a live feed. Consensus = mean no-vig probability across ${ml?.consensus?.books || 0} books quoting both sides. Market intelligence only; PropBetEdge model edges are separate and appear only from a released model.</p>
  </div>`;
}
